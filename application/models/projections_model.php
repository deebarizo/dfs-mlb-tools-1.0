				    ...
				    if ($row != 0) {
				    	...
				    	foreach ($batters as $key => $batter) {
				    		if ($name == $batter['name']) {
						    	...
						    	$this->calculations->stat_projection('bb');
				    	...
				    	foreach ($batters as $key => &$batter) {
				    		if ($name == $batter['name']) {
						    	...
						    	if (isset($projections[$player_id]['games'])) {
							    	...
							    	$projections[$player_id]['bb_steamer'] = ($bb / $games) + (($bb / $games) * $era_mod['bb']);
							    	$projections[$player_id]['hbp_steamer'] = $hbp / $games;
							    	$projections[$player_id]['singles_steamer'] = ($singles / $games) + (($singles / $games) * $era_mod['singles']);
							    	$projections[$player_id]['doubles_steamer'] = (($doubles / $games) + (($doubles / $games) * $era_mod['doubles'])) * 2;
							    	$projections[$player_id]['triples_steamer'] = (($triples / $games) + (($triples / $games) * $era_mod['triples'])) * 3;
